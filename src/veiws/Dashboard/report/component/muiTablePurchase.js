...
const headCells = [
    ...
    { id: 'quantity_ordered', label: 'تعداد/مقدار' },
    { id: 'unit_price', label: 'قیمت' },
    { id: 'total', label: 'ټوټل' },
];
...
export default function PurchaseInvoiceTable({ rows }) {
    // //console.log(rows[0])
 ...
 const totals = useMemo(() => {
     return filteredRows.reduce(
       (acc, row) => {
         ...
         acc.total += Number((row.unit_price * row.quantity_ordered)|| 0);
         return acc;
       },
       { quantity: 0, total: 0 }
     );
        ...
        <TableContainer>
          <Table sx={{ minWidth: 450 }} aria-labelledby="tableTitle" size={'medium'}>
            ...
            {filteredRows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                ...
                <TableCell>{`${row.unit_price}`}</TableCell> {/* Adjust field name */}
                <TableCell>{`${row.quantity_ordered * row.unit_price}`}</TableCell> {/* Adjust field name */}
                </TableRow>
            ))}
            <TableRow>

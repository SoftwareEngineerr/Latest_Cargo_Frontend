...
const ReturnOrdersMain = () => {
    ...
    const fetchOrders = async () => {
        setLoading(true);
        try {
            const res = await dispatch(GetRequest(url.ShowAllReturns, userToken, 'SHOW_CLASSES'));
...
return () => window.removeEventListener('refreshOrders', handleRefreshOrders);
    ...
    return (
        <PageContainer title="Return  راستانه سوی  " description="Orders sent from your branch">
            <Box sx={{ p: 3 }}>
                <Paper
                    elevation={5}
                ...
                >
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                        ...
                        >
                            <LocalShippingIcon color="primary" />Return  راستانه سوی    
                        </Typography>
                        <IconButton
                            onClick={fetchOrders}
                            color="primary"
...
export default ReturnOrdersMain;

...
const DeliveredOrders = () => {
    ...
    useEffect(() => {
        fetchOrders();
            const statusList = [
                { value: 'Cancelled', front: 'Cancelled' },
    ...
    const fetchOrders = async () => {
        setLoading(true);
        try {
            const res = await dispatch(GetRequest(url.GetDeliveredOrder, userToken, 'SHOW_CLASSES'));
...
return () => window.removeEventListener('refreshOrders', handleRefreshOrders);
    ...
    return (
        <PageContainer title="Delivered مکمله سوی پارسلونه  " description="Orders sent from your branch">
            <Box sx={{ p: 3 }}>
                <Paper
                    elevation={5}
                ...
                >
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                        ...
                        >
                            <LocalShippingIcon color="primary" /> Delivered مکمله سوی پارسلونه  
                        </Typography>
                        <IconButton
                            onClick={fetchOrders}
                            color="primary"

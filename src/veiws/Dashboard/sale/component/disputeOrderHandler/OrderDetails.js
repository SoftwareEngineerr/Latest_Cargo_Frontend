...
import OrderDetailView from '../../../purchase/component/muiModelPopup'

import { AddCircle, InventoryOutlined } from '@mui/icons-material';
import { Link } from 'react-router-dom';
import { Custombranch } from '../../../../../components/select/branch';
...
const OrderDetails = ({ order, orderDetail, onStatusRefresh, closeCollaps }) => {
  ...
  const [holdbranch, setHoldbranch] = useState('')
  ...
  const findbranches = () => {
    // const branchitems = JSON.parse(sessionStorage.getItem("branchItems")) || undefined;
    // const paymentinbranch = branchitems.filter((item) => item.srn == getbranch)?.[0]?.address
    // const parcelhold = branchitems.filter((item) => item.srn == holdbranch)?.[0]?.address
    const branchitems = JSON.parse(sessionStorage.getItem("branchItems")) || [];
    const paymentinbranch = branchitems.find((item) => item.srn == getbranch)?.address || "";
    const parcelhold = branchitems.find((item) => item.srn == holdbranch)?.address || "";

    //console.log(paymentinbranch)

    if (paymentinbranch && parcelhold) {
      return [paymentinbranch, parcelhold]
    }
  }
  ...
  const onStatusChange = async (data, value) => {
    ...
    const urlRequest = selectedState == 'return' ? url.ReturnOrder : selectedState == 'delivered' ? url.DeliverdOrder : url.UpdateOrder
    const img = imageUplaoded
    const payload = {
      brsrnForDispute: selectedState == 'return' ? getbranch : data.hold_brsrn,
      findbranches: findbranches(),
      holdbranch: holdbranch,
      order_srn: data.srn, status: selectedState, image: img, comment, track_number: data.track_number, payStatus: data.pay_status,
      customerCharge: selectedState == 'return' ? data.total_fees : data.total_customer_charge, origin: data.origin, branchAddress: data.parcel_destination
    }
  ...
  const branchUpdate = (a, e) => {
    //console.log(a, e)
  }


  const ChangeOnSelect = (getparam, getname) => {
    //console.log(getparam[0].parcel_destination_branch)
    if (getname == 1) {
      setGetbranch(getparam[0].parcel_destination_branch)
    }
    else {
      setHoldbranch(getparam[0].parcel_destination_branch)
    }
      ...
      <Box display="grid" gridTemplateColumns={{ xs: '1fr', sm: '1fr 1fr' }} gap={2}>
        ...
        >
          ...
          <Box>
            ...
            <Typography><b>Date:</b> {order?.order_date?.split('T')[0]}</Typography>
          ...
          {order?.pay_status == 1 ? (
            ...
            >
              <Typography sx={{ color: '#2E7D32', fontWeight: 600 }}>
                💰 Payment Received {parseFloat(order?.total_fees) + parseFloat(order?.shirkat_charges)} AFN
              </Typography>
            </Box>
          ) : null}
        ...
        <Paper sx={{ p: 2, borderRadius: 2, backgroundColor: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
          ...
          {order?.pay_status == 1 ? (
            ...
            <>
              {/* <Custombranch
            GetSelectedValue={(e)=>ChangeOnSelect(e)}
            /> */}
              <Typography>
                <TextField
                  label="Our Fees"
                ...
                />
                {/* <b>Our Fees:</b> {order?.total_fees} AFN */}
              </Typography>
              <Typography>
                <TextField
                  label="Shirkat"
                ...
                {/* <b>Shirkat:</b> {order?.shirkat_charges} AFN */}
              </Typography>
          ...
          <Typography>
            ...
            />
            {/* <b>Parcel Address:</b> {order?.receiver_address} */}
          </Typography>
        ...
        <Paper sx={{ p: 2, borderRadius: 2, backgroundColor: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }} elevation={0}>
          ...
          <Box mt={1}>
              ...
              selectedState == 'return' ?
                <>
                  <Typography textAlign="right">
                    کرایه   :{changeorder?.total_fees}
                  </Typography>
                  <Typography textAlign="right">
                    مهرباني وکړئ هغه څانګه غوره کړئ چې کرایه راټولوي.              </Typography>
                  <Custombranch
                    GetSelectedValue={(e) => ChangeOnSelect(e, 1)}
                  />
                  <br />

                  <Typography textAlign="right">
                    اوس محصول دې څانګې ته سپارل کیږي.
                  </Typography>
            ...
            <br />
            <br />
            <CustomBtn mt={2} disable={statuses.length === 0 || selectedState == undefined ? true : false} data="Update Status" click={() => onStatusChange(order, 'customer_status')} />

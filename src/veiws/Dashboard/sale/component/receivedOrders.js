...
const ReceivedOrders = () => {
    ...
    useEffect(() => {
        ...
        const statusList = [
            ...
            { value:'delivered', front:'Delivered'},
            { value: 'dispute', front: 'Dispute' }
        ];
    ...
    const fetchOrders = async () => {
        setLoading(true);
        try {
            const res = await dispatch(GetRequest(url.GetReceivedOrder, userToken, 'SHOW_CLASSES'));
...
return () => window.removeEventListener('refreshOrders', handleRefreshOrders);
    ...
    return (
        <PageContainer title="Received را رسیدلي پارسلونه " description="Orders sent from your branch">
            <Box sx={{ p: 3 }}>
                <Paper
                    elevation={5}
                ...
                >
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                        ...
                        >
                            <LocalShippingIcon color="primary" /> Received را رسیدلي پارسلونه  
                        </Typography>
                        <IconButton
                            onClick={fetchOrders}
                            color="primary"

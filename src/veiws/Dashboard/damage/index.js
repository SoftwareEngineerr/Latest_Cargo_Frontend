...
const Damage = () => {
    ...
    const fetchOrders = async () => {
        setLoading(true);
        try {
            const res = await dispatch(GetRequest(url.MissingDamageAdmin, userToken,''));
...
return () => window.removeEventListener('refreshOrders', handleRefreshOrders);
    ...
    return (
        <PageContainer title="Damage/Missing   زیان یا ورک شوی  " description="Orders sent from your branch">
            <Box sx={{ p: 3 }}>
                <Paper
                    elevation={5}
                ...
                >
                    <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                        ...
                        >
                            <LocalShippingIcon color="primary" /> Damage/Missing   زیان یا ورک شوی  
                        </Typography>
                        <IconButton
                            onClick={fetchOrders}
                            color="primary"

...
const Auth = (props) => {
  ...
  const payload = {
    ...inputValues,
    api: apilink,
    WebSrn: JSON.parse(localStorage.getItem("WebSrn")),
  ...
  try {
    ...
    // If you're returning something like { success: true } from loginauth
    if (result?.success) {
      setRes(true);
      localStorage.setItem('BranchDetails', JSON.stringify(result?.data?.branch))
      sessionStorage.setItem('BranchDetails', JSON.stringify(result?.data?.branch))
      localStorage.setItem('BranchID', result?.data?.branch?.srn)
